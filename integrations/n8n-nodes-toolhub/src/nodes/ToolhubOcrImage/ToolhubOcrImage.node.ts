...
export class ToolhubOcrImage implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Toolhub OCR Image',
		name: 'toolhubOcrImage',
		icon: 'fa:file',
		group: ['transform'],
		version: 1,
		description: 'Run image OCR through Toolhub run-file',
		defaults: {
			name: 'Toolhub OCR Image',
		},
		inputs: ['main'],
		outputs: ['main'],
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		return executeFileToolNode(this, { toolAlias: 'n8n_ocr_image' });
	}
}
