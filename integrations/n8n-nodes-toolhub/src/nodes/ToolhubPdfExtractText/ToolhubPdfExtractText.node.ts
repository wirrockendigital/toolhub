...
export class ToolhubPdfExtractText implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Toolhub PDF Extract Text',
		name: 'toolhubPdfExtractText',
		icon: 'fa:file',
		group: ['transform'],
		version: 1,
		description: 'Extract text from PDF files through Toolhub run-file',
		defaults: {
			name: 'Toolhub PDF Extract Text',
		},
		inputs: ['main'],
		outputs: ['main'],
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		return executeFileToolNode(this, { toolAlias: 'n8n_pdf_extract_text' });
	}
}
